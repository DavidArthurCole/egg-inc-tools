...
export const boostCombinations = [
  ...
  return {
    ...
    newTokens: (prisms.reduce((sum, prism) => sum + (prism.newTokens !== undefined ? prism.newTokens : prism.tokens), 0)) +
      (beacons.reduce((sum, beacon) => sum + (beacon.newTokens !== undefined ? beacon.newTokens : beacon.tokens), 0)),
    time: comboTime,
    name: [
    ...
    chickensForHatchRate(hatchRate, {dilithiumBoostBonus = 0 } = {}, { artifactBoostBoostBonus = 0 } = {}, { doubleBoostLength = false} = {}) {
      ...
      for (let t = timeStep; t <= comboTime; t += timeStep) {
        ...
        chickensHatched += hatchRate * boostMultiplier * dilithiumBoostBonus * timeStep * (doubleBoostLength ? 2 : 1)
      }
